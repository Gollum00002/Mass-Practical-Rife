...
> **Linux & Windows** — MPR supports both Linux and Windows. Before proceeding, follow the included **`Install-Instructions.txt`** to set up your environment.
...
> This is a hobbyist project maintained in my spare time. Updates will be infrequent — don't expect regular releases.
...
Follow **`Install-Instructions.txt`** included in this repository before doing anything else. It covers setting up a Python virtual environment, installing PyTorch, and all required dependencies — for both Linux and Windows.

### Configuration

Open `Launch.sh` (Linux) or `Launch.bat` (Windows) and set the following parameters before running:
...
| `OUTPUT_DIR` | Path/name of the output folder. If no path is given, the folder is created in the same directory as the launch script |
