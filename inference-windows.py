...
def transferAudio(sourceVideo, targetVideo):
    ...
    try:
        ...
        if audio_check.returncode == 0:
            ...
            # Using communicate() prevents deadlock if the output buffer gets too full
            out, err = process.communicate()
            
            if process.returncode != 0:
                raise Exception(f"Audio transfer failed. FFmpeg error: {err}")
...
def process_video(video_path, output_dir, target_fps, modelDir='train_log', fp16=False, scale=1.0, ext='mp4', png=False, auto_scale=True):
    ...
    try:
        ...
        if fp16 and not torch.cuda.is_available():
            tqdm.write("  -> CUDA not available. Disabling FP16 to prevent CPU crash.", file=sys.stdout)
            fp16 = False
